...
We see that we can execute the `joe` binary with `root` privileges.
We execute it since we don't know what it is.
...
We discover that by pressing `Ctrl + K`, followed by `!`, a prompt appears to execute a command.
