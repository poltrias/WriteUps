...
> **Tags:** `Windows` `Active Directory` `SMB` `Null Session` `RID Cycling` `Password Spraying` `Kerberoasting` `Pass-the-Hash` `WinRM`
...
This does not help us move forward. Now that we have valid credentials, we proceed to test additional attack paths such as a Kerberoasting attack.
...
We manage to recover one plaintext password from these hashes, but we do not initially know which account it belongs to. To identify the correct user, we run `password spraying` again.
...
This confirms the plaintext credentials for user `file_svc`. Next, we check whether the `file_svc` account can access the SMB share `backup`.

## SMB with user file\_svc
