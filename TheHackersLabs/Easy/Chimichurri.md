...
Vemos claramente que se trata de un active directory. a parte de los puertos tipicos de AD encontramos un jenkins en el puerto 6969.

Vemos que el dominio es chimichurri.thl
...
Encontramos lo que parecen unas credenciales para el usuario hacker:perico. Pero no son vvalidas ni para smb ni para winrm.

Pasamos a ver que podemos hacer con el jenkins del puerto 6969.




En la parte de abajo vemos la vversion: Jenkins 2.361.4

Buscamos vvulns asociadas a esta verrsion.

Encontramos una vulnerabilidad de file read parra esta version.

Descargamos el exploit a la maquina atacante.
...
Encontramos las credenciales reales para el usuario hacker : Perico69.
...
Comprobamos privilegios del usuario hacker.
...
Tenemos el privilegio SeImpersonatePrivilege  activado.

Bajamos PetitPotato.exe en nuestra maquina atcante.

Creamos un directorio temp en la maquina victima para subir el exploit.
...
La subimos a la maquina victima
...
Ejecutamos el exploit y lo usamos para crear un nuevo usuario llamado hackeado con password Password123.

```
./PetitPotato.exe -c "net user pwned Password123 /add"
```
...
Ahora metemos este nuevo usuario en el grupo de administradores.
...
Ahora que tenemos un usuario administrador del cual sabemos usuario y contraseña, podemos utilizar impacket-secretsdump para volcar todos los hashes del sistema.
...
Utilizamos el hash de dominio del usuario Administrador para autenticarnos via WINRM.
...
Ya estamos como usuario administrador.
